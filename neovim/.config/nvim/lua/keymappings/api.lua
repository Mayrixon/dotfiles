...
local function set_lsp_leader_mappings(bufnr)
  ...
  wk.register(
    keymappings.lsp_keys.leader.normal_mode,
    utils.get_buffer_opts(bufnr, utils.get_prefix_opts("<leader>", keymappings.opts.leader.normal))
  )
  wk.register(
    keymappings.lsp_keys.leader.visual_mode,
    utils.get_buffer_opts(bufnr, utils.get_prefix_opts("<leader>", keymappings.opts.leader.visual))
  )
end
local function set_lsp_capability_mappings(client)
  ...
  for _, m in pairs(keymappings.lsp_keys.capability_mappings) do
    ...
    if client.resolved_capabilities[capability] then
      wk.register(key, utils.get_prefix_opts(prefix, keymappings.opts.leader[mode]))
    end
  end
end
