...
M.theme_path = MyVim.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
...
function M.open(opts)
  ...
  if vim.g.lazygit_config then
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        vim.env.LG_CONFIG_FILE = MyVim.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
      else
  ...
  return MyVim.terminal(cmd, opts)
end
...
function M.get_color(v)
  ...
  local color = {}
  if v.fg then
    color[1] = MyVim.ui.color(v.fg)
  elseif v.bg then
    color[1] = MyVim.ui.color(v.bg, true)
  end
  if v.bold then
...
function M.blame_line(opts)
  ...
  local root = MyVim.root.detectors.pattern(0, { ".git" })[1]
